...
const CartPage = () => {
  ...
  return (
    <div className="container mx-auto font-poppins p-10 lg:px-20 pb-20">
      {products.length > 0 ? (
        <div className="lg:flex lg:flex-row lg:space-x-6">
          <div className="flex-1 mb-6">
            ...
            <div className="flex-col sm:flex-row">
              {products.map(({ product, quantity }) => (
                <div key={product.sku} className="flex flex-col sm:flex-row items-center mb-4 pb-4 sm:items-center">
                  <div className="w-full sm:w-1/3 flex flex-col sm:flex-row items-center">
                    ...
                    <div className="sm:ml-4 mt-2 sm:mt-0">
                      <h3 className="font-semibold text-gray text-center sm:text-left lg:ml-14">{product.name}</h3>
                    </div>
                  </div>
                  <p className="w-full sm:w-1/6 text-center text-gray-600 mt-2 sm:mt-0 sm:block hidden text-gray">Rs. {product.price.toLocaleString()}</p>
                  ...
                  <p className="w-full sm:w-1/6 text-center sm:text-right mt-2 sm:mt-0 sm:ml-10">Rs. {(product.price * quantity).toLocaleString()}</p>
                  <button onClick={() => handleRemoveProduct(product.sku)} className="ml-4 mt-2 sm:mt-0 text-red-500">
                    <img className="lg:ml-10 mr-2" src="https://desafio3furniro.s3.us-east-2.amazonaws.com/cart/lixeira.png" alt="lixeira" />
                  </button>
                </div>
              ))}
            </div>
          ...
          <div className="hidden lg:block w-1/3 bg-filter p-2 pl-12 pr-12 ml-6 h-[300px] sm:h-80">
            <h2 className="text-2xl font-semibold mb-12 text-center">Cart Totals</h2>
